...
Take Snapshotで取得した最新のスナップショットを表示します。  
デフォルトでは前回取得時からの差分を取得、ランクマップは増えた場合は前回取得時の2か月前から取得します。  
うまくランクが反映されない場合は、Score Fetch ModeでFetch ALLをチェックしてスナップショットを取得してください。  
また、Acc Saberの国別ランキングのみFetch Ranking Dataで取得したindex(プレイヤーの国を紐づける)ファイルを使用します。  
